...
return {
	...
	dependencies = {
		...
		-- Declared here too (not just in autocompletion_blink.lua) so lazy.nvim guarantees
		-- blink.cmp is loaded before this config() runs, regardless of file/require order.
		"saghen/blink.cmp",
	},
	config = function()
		...
		-- blink.cmp already builds on Neovim's default capabilities and extends them, so
		-- there's no need to build a base table ourselves like the nvim-cmp variant does.
